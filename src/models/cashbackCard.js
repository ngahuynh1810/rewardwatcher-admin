...
const cashbackCard = {
    ...
    effects: (dispatch) => ({
      ...
      async getList(payload, rootState) {
        try {
          let response = await axios.get(`${API_URL}cashback-card/list?${createSearchParams(payload)}`, {
            headers: {
          ...
          if(isSuccessResult(response)) {
            this.setState({
              listCashbackCard: result?.data?.list,
              totalCashbackCard: result?.data?.total,
            })
          }
      ...
      async create(payload, rootState) {
        try {
          let response = await axios.post(`${API_URL}cashback-card/create`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async getDetail(payload, rootState) {
        try {
          let response = await axios.get(`${API_URL}cashback-card/detail/${payload.id}`, {
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
      ...
      async update(payload, rootState) {
        try {
          let response = await axios.put(`${API_URL}cashback-card/update/${payload?.uuid}`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async delete(payload, rootState) {
        try {
          let response = await axios.delete(`${API_URL}cashback-card/delete/${payload.id}`   ,{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
