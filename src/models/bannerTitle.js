...
const bannerTitle = {
    ...
    effects: (dispatch) => ({
      ...
      async getList(payload, rootState) {
        try {
          let response = await axios.get(`${API_URL}bannerTitle/list?${createSearchParams(payload)}`, {
            headers: {
          ...
          if(isSuccessResult(response)) {
            this.setState({
              listBannerTitle: result?.data?.list,
              totalBannerTitle: result?.data?.total,
            })
          }
      ...
      async create(payload, rootState) {
        try {
          let response = await axios.post(`${API_URL}bannerTitle/create`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async getDetail(payload, rootState) {
        try {
          let response = await axios.get(`${API_URL}bannerTitle/detail/${payload.id}`, {
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
      ...
      async update(payload, rootState) {
        try {
          let response = await axios.put(`${API_URL}bannerTitle/update/${payload?.uuid}`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async delete(payload, rootState) {
        try {
          let response = await axios.delete(`${API_URL}bannerTitle/delete/${payload.id}`   ,{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
