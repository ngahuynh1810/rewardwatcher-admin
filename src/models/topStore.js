...
const topStore = {
    ...
    effects: (dispatch) => ({
      ...
      async getList(payload, rootState) {
        try {
          let response = await axios.get(`${API_URL}top-store/list?${createSearchParams(payload)}`, {
            headers: {
          ...
          if(isSuccessResult(response)) {
            this.setState({
              listTopStore: result?.data?.list,
              totalTopStore: result?.data?.total,
            })
          }
      ...
      async create(payload, rootState) {
        try {
          let response = await axios.post(`${API_URL}top-store/create`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async getDetail(payload, rootState) {
        try {
          let response = await axios.get(`${API_URL}top-store/detail/${payload.id}`, {
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
      ...
      async update(payload, rootState) {
        try {
          let response = await axios.put(`${API_URL}top-store/update/${payload?.uuid}`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async delete(payload, rootState) {
        try {
          let response = await axios.delete(`${API_URL}top-store/delete/${payload.id}`   ,{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
