...
const sessionDeal = {
    ...
    effects: (dispatch) => ({
      ...
      async getList(payload, rootState) {
        try {
          let response = await axios.get(`${API_URL}season-deal/list?${createSearchParams(payload)}`, {
            headers: {
          ...
          if(isSuccessResult(response)) {
            this.setState({
              ...
              totalSessionDeal: result?.data?.total,
            })
          }
      ...
      async create(payload, rootState) {
        try {
          let response = await axios.post(`${API_URL}season-deal/create`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async getDetail(payload, rootState) {
        try {
          let response = await axios.get(`${API_URL}season-deal/detail/${payload.id}`, {
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
      ...
      async update(payload, rootState) {
        try {
          let response = await axios.put(`${API_URL}season-deal/update/${payload?.uuid}`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async delete(payload, rootState) {
        try {
          let response = await axios.delete(`${API_URL}season-deal/delete/${payload.id}`   ,{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
