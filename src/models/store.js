    ...
    effects: (dispatch) => ({
      ...
      async getList(payload, rootState) {
        ...
        console.log(filterParam)
          let response = await axios.get(`${API_URL}stores/list?${createSearchParams(payload)}&filter=${filterParam}`, {
            headers: {
          ...
          if(isSuccessResult(response)) {
            this.setState({
              listStore: result?.data?.list,
              totalStore: result?.data?.total,
            })
          }
      ...
      async create(payload, rootState) {
        try {
          let response = await axios.post(`${API_URL}stores/create`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async getDetail(payload, rootState) {
        try {
          let response = await axios.get(`${API_URL}stores/detail/${payload.id}`, {
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
      ...
      async update(payload, rootState) {
        try {
          let response = await axios.put(`${API_URL}stores/update/${payload?.uuid}`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async delete(payload, rootState) {
        try {
          let response = await axios.delete(`${API_URL}stores/delete/${payload.id}`   ,{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
