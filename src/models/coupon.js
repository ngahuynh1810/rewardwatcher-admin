...
const coupon = {
    ...
    effects: (dispatch) => ({
      ...
      async getList(payload, rootState) {
        try {
          let response = await axios.get(`${API_URL}coupon/list?${createSearchParams(payload)}`, {
            headers: {
      ...
      async create(payload, rootState) {
        try {
          let response = await axios.post(`${API_URL}coupon/create`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async getDetail(payload, rootState) {
        try {
          let response = await axios.get(`${API_URL}coupon/detail/${payload.id}`, {
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
      ...
      async update(payload, rootState) {
        try {
          let response = await axios.put(`${API_URL}coupon/update/${payload?.uuid}`, {
            ...payload
          },{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
      ...
      async delete(payload, rootState) {
        try {
          let response = await axios.delete(`${API_URL}coupon/delete/${payload.id}`   ,{
            headers: {
              Authorization: `Bearer ${loadCookie(LOGIN_TOKEN)}`
            }
          });  
