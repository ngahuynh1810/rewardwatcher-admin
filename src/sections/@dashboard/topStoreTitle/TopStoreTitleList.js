...
function TopStoreTitleList(props) {
    ...
    useEffect(() => {
        getListTopStoreTitle()
    }, [location?.search]);

    const onFilterChange = React.useCallback((filterModel) => {
        // navigate(`/dashboard/top-store-title?page=${event.page}&limit=${event.pageSize}`)
    ...
    const handleEditTopStoreTitle = (itemSelected) => {
        setItemSelected(itemSelected)
        navigate(`/dashboard/top-store-title/${itemSelected.id}`)
    }
    const getListTopStoreTitle = async() => {
      setLoading(true)
      let query = {
        ...
        if (sort) query.sort = sortBy
        if (sortBy) query.sort_by = sort
        props.getListTopStoreTitle(query);
        setLoading(false)
    }
    useEffect(() => {
    ...
    return (
      ...
      <DataGrid
          ...
          //    autoPageSize={true}
          //    rowCount={props.totalTopStoreTitle}
          ...
          //    rowBuffer={props.totalTopStoreTitle}
          rowCount={props.totalTopStoreTitle}
            filterMode="server"
...
const mapState = (state) => ({
    ...
    totalTopStoreTitle: state.topStoreTitle.totalTopStoreTitle,
});
const mapDispatch = (dispatch) => ({
    getListTopStoreTitle: dispatch.topStoreTitle.getList
});
export default connect(mapState, mapDispatch)(TopStoreTitleList)
