...
function CashbackCardTitleList(props) {
    ...
    useEffect(() => {
        getListCashbackCardTitle()
    }, [location?.search]);

    const onFilterChange = React.useCallback((filterModel) => {
        // navigate(`/dashboard/cashback-card-title?page=${event.page}&limit=${event.pageSize}`)
    ...
    const handleEditCashbackCardTitle = (itemSelected) => {
        setItemSelected(itemSelected)
        navigate(`/dashboard/cashback-card-title/${itemSelected.id}`)
    }
    const getListCashbackCardTitle = async() => {
      setLoading(true)
      let query = {
        ...
        props.getListCashbackCardTitle(query);
        setLoading(false)
    }
    useEffect(() => {
    ...
    return (
      ...
      <DataGrid
          {...data}
          columns={columns}
          rows={props.listCashbackCardTitle || []}
          initialState={{
            columns: {
          ...
          //    autoPageSize={true}
          //    rowCount={props.totalCashbackCardTitle}
          ...
          //    rowBuffer={props.totalCashbackCardTitle}
          rowCount={props.totalCashbackCardTitle}
            filterMode="server"
...
const mapState = (state) => ({
    ...
    totalCashbackCardTitle: state.cashbackCardTitle.totalCashbackCardTitle,
});
const mapDispatch = (dispatch) => ({
    getListCashbackCardTitle: dispatch.cashbackCardTitle.getList
});
export default connect(mapState, mapDispatch)(CashbackCardTitleList)
